""" Tests for outliers_utils module

Run with:

    nosetests test_outliers_utils
"""

import numpy as np

from .. import outliers_utils
...
def test_iqr_outliers():
    ...
    exp_lo = 25 - 75
    exp_hi = 75 + 75
    indices, thresholds = outliers_utils.iqr_outliers(arr)
    assert_array_equal(indices, [])
    assert_equal(thresholds, (exp_lo, exp_hi))
    # Reverse, same values
    indices, thresholds = outliers_utils.iqr_outliers(arr[::-1])
    assert_array_equal(indices, [])
    assert_equal(thresholds, (exp_lo, exp_hi))
    # Add outliers
    arr[0] = -51
...
def test_iqr_scaling():
    ...
    exp_lo = 25 - 100
    exp_hi = 75 + 100
    indices, thresholds = outliers_utils.iqr_outliers(arr, 2)
    assert_array_equal(indices, [])
    assert_equal(thresholds, (exp_lo, exp_hi))
...
def test_vol_rms_diff():
    ...
    actual_rms = outliers_utils.vol_rms_diff(arr_4d)
...
def test_extend_diff_outliers():
    ...
    assert_array_equal(outliers_utils.extend_diff_outliers(indices),
                       [3, 4, 7, 8, 12, 13, 20, 21])
...
def test_vol_std():
    ...
    # Reshape to 4D
    arr_4d = np.reshape(arr_2d, shape_3d + (T,))
    actual_stds = outliers_utils.vol_std(arr_4d)
